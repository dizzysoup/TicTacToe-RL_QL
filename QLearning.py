...
class Qlearning:
    # epsilon、alpha 、 gamma 
    def __init__(self,epsilon=0.2, alpha=0.1, gamma=0.9, stop_training=False, total_episodes=200000):
    ...
    def game_begin(self):
        ...
        self.q_last = 0.0
        self.state_action_last = None
    # 用 epsilon-greedy 策略選擇動作，根據一定epsilon 機率隨機選擇動作，否則選擇Q值最大的動作
    def epslion_greedy(self, state, possible_moves): #esplion greedy algorithm
        #return  action
        self.episodes += 1
        self.last_board = tuple(state)
        if(self.stop_training and self.episodes >= self.total_episodes): self.epsilon = 0.0
